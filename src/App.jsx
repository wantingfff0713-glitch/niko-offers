...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ display:"inline-flex", alignItems:"center", gap:5, background:cfg.bg, color:cfg.color, padding:"3px 10px", borderRadius:20, fontSize:12, fontWeight:600 }}>
...
const IS = { width:"100%", padding:"8px 10px", borderRadius:8, border:"1px solid #e5e7eb", fontSize:13, fontFamily:"inherit", outline:"none", boxSizing:"border-box" };
...
function EditForm({ offer, onSave, onCancel }) {
  const [form, setForm] = useState({
    property: {
      address:        offer.property?.address        || "",
      postcode:       offer.property?.postcode       || "",
      weeklyRent:     offer.property?.weeklyRent     || "",
      monthlyRent:    offer.property?.monthlyRent    || "",
      startDate:      offer.property?.startDate      || "",
      tenancyType:    offer.property?.tenancyType    || "",
      reservationFee: offer.property?.reservationFee || "",
      deposit:        offer.property?.deposit        || "",
    },
    tenant: { name: offer.tenant?.name || "", email: offer.tenant?.email || "", phone: offer.tenant?.phone || "" },
    agent:  { name: offer.agent?.name  || "", company: offer.agent?.company || "", email: offer.agent?.email || "", phone: offer.agent?.phone || "" },
    notes: offer.notes || "",
  });
  ...
  return (
    <div>
      <SH title="🏢 Property" />
      <FW label="Address"><input style={IS} value={form.property.address} onChange={e => updateField("property","address",e.target.value)} /></FW>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <FW label="Postcode"><input style={IS} value={form.property.postcode} onChange={e => updateField("property","postcode",e.target.value)} /></FW>
        <FW label="Tenancy Type"><input style={IS} value={form.property.tenancyType} onChange={e => updateField("property","tenancyType",e.target.value)} /></FW>
        <FW label="Weekly Rent (£)"><input style={IS} type="number" value={form.property.weeklyRent} onChange={e => updateField("property","weeklyRent",e.target.value)} /></FW>
        <FW label="Monthly Rent (£)"><input style={IS} type="number" value={form.property.monthlyRent} onChange={e => updateField("property","monthlyRent",e.target.value)} /></FW>
        <FW label="Start Date"><input style={IS} value={form.property.startDate} onChange={e => updateField("property","startDate",e.target.value)} /></FW>
        <FW label="Reservation Fee (£)"><input style={IS} type="number" value={form.property.reservationFee} onChange={e => updateField("property","reservationFee",e.target.value)} /></FW>
      </div>
      <FW label="Deposit (£)"><input style={IS} type="number" value={form.property.deposit} onChange={e => updateField("property","deposit",e.target.value)} /></FW>
      <SH title="👤 Tenant" />
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <FW label="Name"><input style={IS} value={form.tenant.name} onChange={e => updateField("tenant","name",e.target.value)} /></FW>
        <FW label="Phone"><input style={IS} value={form.tenant.phone} onChange={e => updateField("tenant","phone",e.target.value)} /></FW>
      </div>
      <FW label="Email"><input style={IS} value={form.tenant.email} onChange={e => updateField("tenant","email",e.target.value)} /></FW>
      <SH title="🏛 Agent" />
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        <FW label="Agent Name"><input style={IS} value={form.agent.name} onChange={e => updateField("agent","name",e.target.value)} /></FW>
        <FW label="Company"><input style={IS} value={form.agent.company} onChange={e => updateField("agent","company",e.target.value)} /></FW>
        <FW label="Agent Email"><input style={IS} value={form.agent.email} onChange={e => updateField("agent","email",e.target.value)} /></FW>
        <FW label="Agent Phone"><input style={IS} value={form.agent.phone} onChange={e => updateField("agent","phone",e.target.value)} /></FW>
      </div>
      <SH title="📝 Notes" />
      <FW label="Additional Notes">
        <textarea style={{ ...IS, minHeight:70, resize:"vertical" }} value={form.notes} onChange={e => setForm(f => ({ ...f, notes:e.target.value }))} />
      </FW>
      <div style={{ display:"flex", gap:10, marginTop:20 }}>
        <button onClick={onCancel} style={{ padding:"10px 16px", background:"#f3f4f6", color:"#374151", border:"none", borderRadius:10, fontWeight:600, fontSize:13, cursor:"pointer" }}>Cancel</button>
        <button onClick={handleSave} style={{ flex:1, padding:10, background:"#111", color:"#fff", border:"none", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer" }}>Save Changes</button>
      </div>
    </div>
  );
}

function OfferModal({ offer, onClose, onUpdateStatus, onDelete, onEdit }) {
      ...
      <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:540, maxHeight:"88vh", overflowY:"auto", padding:"28px", boxShadow:"0 25px 60px rgba(0,0,0,0.25)" }} onClick={e => e.stopPropagation()}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:16 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {!editing && (
              <button onClick={() => setEditing(true)} style={{ fontSize:12, fontWeight:600, color:"#374151", background:"#f3f4f6", border:"none", borderRadius:8, padding:"6px 12px", cursor:"pointer" }}>✏️ Edit</button>
            )}
        ...
        {editing ? (
          <EditForm offer={offer} onSave={(updated) => { onEdit(updated); setEditing(false); }} onCancel={() => setEditing(false)} />
        ) : (
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
              {[
                ...
                <div key={label} style={{ background:"#f0fdf4", borderRadius:10, padding:"12px 14px", textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:15, fontWeight:800, color:"#065f46", marginTop:4 }}>{value}</div>
                </div>
              ) : null)}
            </div>
            ...
            {offer.files && offer.files.length > 0 && (
              <Section icon="📎" title={`Attachments (${offer.files.length})`}>
                <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                  {offer.files.map((f, i) => <div key={i} style={{ fontSize:12, background:"#e5e7eb", padding:"4px 10px", borderRadius:6, color:"#374151" }}>{f.name}</div>)}
                </div>
              </Section>
            )}
            <div style={{ marginTop:16 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {Object.keys(STATUS_CONFIG).map(s => (
                  <button key={s} onClick={() => onUpdateStatus(offer.id, s)} style={{ padding:"5px 12px", borderRadius:20, fontSize:12, fontWeight:600, cursor:"pointer", border: offer.status === s ? "2px solid #111" : "2px solid #e5e7eb", background: offer.status === s ? "#111" : "#fff", color: offer.status === s ? "#fff" : "#374151" }}>{s}</button>
                ))}
              </div>
            </div>
