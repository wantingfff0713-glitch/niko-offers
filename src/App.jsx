        ...
        {offer.files && offer.files.length > 0 && (
          <Section icon="📎" title={`Attachments (${offer.files.length})`}>
            <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
              {offer.files.map((f, i) => (
                <div key={i} style={{ fontSize:12, background:"#e5e7eb", padding:"4px 10px", borderRadius:6, color:"#374151" }}>{f.name}</div>
              ))}
            </div>
          </Section>
        )}
...
function AddOfferModal({ onClose, onSave }) {
  ...
  async function handleParse() {
    ...
    try {
      ...
      setError("Failed to parse. Please check your API key is set in Vercel, or fill in manually.");
      setStep("input");
    }
  }
          ...
          <div>
            <FW label="Agent Confirmation Email (paste full text)">
              <textarea value={emailText} onChange={e => setEmailText(e.target.value)}
                placeholder="Paste the full email here — AI will extract address, rent, dates, tenant and agent details automatically…"
                style={{ ...IS, minHeight:200, resize:"vertical", lineHeight:1.6 }} />
            </FW>
        ...
        {step === "review" && (
          <div>
            ...
            <FW label="Additional Notes">
              <textarea style={{ ...IS, minHeight:70, resize:"vertical" }} value={form.notes} onChange={e => setForm(f => ({ ...f, notes:e.target.value }))} placeholder="Guarantor needed, special conditions, parking, etc." />
            </FW>
      ...
      <div style={{ background:"#111", color:"#fff", padding:"20px 24px 0" }}>
        <div style={{ maxWidth:720, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
            ...
            <button onClick={() => setShowAdd(true)} style={{ background:"#fff", color:"#111", border:"none", borderRadius:10, padding:"9px 18px", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
              + New Offer
            </button>
          </div>
